...
export default function ChinhSuaClient({ item }: { item: NewsRow }) {
  ...
  const [fbUrl, setFbUrl] = useState(item.fb_url ?? '')
  const [tag, setTag] = useState(item.tag)
  const [tagColor, setTagColor] = useState(item.tag_color)
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      <header className="bg-white shadow-sm sticky top-0 z-10">
        <div className="max-w-3xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h1 className="font-bold text-gray-900">Chỉnh sửa bài đăng</h1>
          </div>
          <button
            form="edit-form"
            type="submit"
      ...
      <form id="edit-form" onSubmit={handleSubmit} className="max-w-3xl mx-auto px-4 py-6 space-y-5">
        {error && (
